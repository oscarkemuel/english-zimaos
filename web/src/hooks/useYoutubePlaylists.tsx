...
const useYoutubePlaylists = () => {
  ...
  const playlists =
    storedPlaylists as YoutubePlaylist[];

  const watched =
    storedWatched as YoutubeWatchedVideos;
  ...
  const addPlaylist = async (
    ...
    const playlist =
      await fetchYoutubePlaylist(value);
  ...
  const toggleVideoWatched = (
    ...
    const playlistWatched =
      watched[playlistId] ?? [];

    const isAlreadyWatched =
      playlistWatched.includes(
        videoId,
      );

    const updatedWatched =
      isAlreadyWatched
        ? playlistWatched.filter(
            (id) => id !== videoId,
          )
        : [
            ...playlistWatched,
